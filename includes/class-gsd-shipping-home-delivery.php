...
/**
 * Home Delivery Shipping Method Class
 */
class GSD_Shipping_Home_Delivery extends WC_Shipping_Method {

    /**
     * Constructor
     */
    public function __construct($instance_id = 0) {
        $this->id = 'gsd_home_delivery';
        $this->instance_id = absint($instance_id);
        $this->method_title = __('Home Delivery', 'garden-sheds-delivery');
        $this->method_description = __('Home delivery shipping option for garden sheds', 'garden-sheds-delivery');
        $this->supports = array(
            'shipping-zones',
            'instance-settings',
        );
    ...
    public function init_form_fields() {
        $this->instance_form_fields = array(
            ...
            'title' => array(
                ...
                'default' => __('Home Delivery', 'garden-sheds-delivery'),
                'desc_tip' => true,
            ),
        );
    }
    ...
    public function calculate_shipping($package = array()) {
        // Check if cart has products with home delivery enabled
        if (!$this->cart_has_home_delivery($package)) {
            return;
        }
    ...
    /**
     * Check if cart has home delivery option
     *
     * @param array $package Package information
     * @return bool
     */
    private function cart_has_home_delivery($package) {
